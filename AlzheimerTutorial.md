...
Once the files `Alzheimer_Interactions.RData` and `functionsVariational.R` have been downloaded, set the working directory to the folder where they are located. Then, clean the workspace and load the data `Alzheimer_Interactions.RData` along with the source file `functionsVariational.R` and other useful packages.
